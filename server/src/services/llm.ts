...
function buildUserPrompt(video: Video): string {
  const lines = [
    'Video metadata:',
    `- title: ${video.title}`,
...
/**
 * Minimal OpenAI-compatible chat completions client. Works with OpenAI, DeepSeek,
 * and most providers that expose a /chat/completions endpoint.
 */
export async function requestAnalysis(video: Video, config: LlmConfig, timeoutMs = 30000): Promise<AnalysisResult> {
